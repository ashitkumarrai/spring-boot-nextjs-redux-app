...
const CourseFlowSidePane = ({ batch_id, isPool }) => {
    const { setOpenModal } = useContext(ManagedUI);
    const [tempRow, setTempRow] = useState([]);
    useEffect(() => {
        async function fetch() {
            if (isPool) {
                const res = await getPoolDetails(batch_id);

                if (res.status == 200) {
                    setTempRow(res.data.data.pool_course_flows);
                    console.log(res.data.data.pool_course_flows);
                }
            } else {
...
CourseFlowSidePane.defaultProps = {
    isPool: false,
};
