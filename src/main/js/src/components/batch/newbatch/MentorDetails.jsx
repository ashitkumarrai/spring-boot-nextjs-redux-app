...
//context
//API
import { getMentorByBatch } from "@services/Api/batch.api";
export default function MentorDetails({
    ...
    useEffect(() => {
        async function fetch() {
            ...
            const MentorRes = await getMentorByBatch({
                batch_id: batch_id,
            ...
            //  console.log("MentorRes", MentorRes.data.slot);
            let tempSlots = [];
    ...
    useEffect(() => {
        ...
        if (selectedDay !== undefined) {
            ...
            const filteredCourseFlow = tempRow.filter(
                (entry) =>
                    moment(entry.date).format("dddd").toLowerCase() ===
                        tempSelectedDay.toLowerCase() &&
        ...
        <SidePane position="right" padding="0" width="28%">
            <div className={style.container}>
                ...
                <span className={style.MentorHeader}>
                    ...
                    <p className={style.MentorSubjectText}>{clickedSubject}</p>
                </span>
                {/* Mentor Details */}
                ...
                >
                    {filterCourseFlowArray.length > 0 ? (
                        filterCourseFlowArray.map((element, key) => {
                            if (element.subject_name === clickedSubject) {
                                return (
                                    <CourseFlowRow
                                        key={key}
                                        data={element}
